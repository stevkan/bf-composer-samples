...
namespace GaiaV2CustomActions.Middleware
{
    ...
    public class TurnStateSetupMiddleware : IMiddleware
    {
        ...
        {
            if(turnContext.Activity.Type == ActivityTypes.Message && turnContext.Activity.Text != "FORCESIGNIN" && !string.IsNullOrEmpty(token))
            {
                if(update)
                {
        ...
        private async Task<string> GetAuthToken(ITurnContext turnContext, string magicCode, CancellationToken cancellationToken = default)
        {
            ...
            {
                tokenResponse = await userTokenClient.GetUserTokenAsync(turnContext.Activity.From?.Id, "KanOAuth", turnContext.Activity.ChannelId, magicCode, cancellationToken).ConfigureAwait(false);
            }
